...
# 데이터 저장 =====================
# os.chdir('C:/New Project/2. 유일로보틱스 과제/차선 촬영 영상/')  # 디렉토리
...
# 중앙 =======
# cap_c = cv2.VideoCapture('20221013 차선 촬영_중앙_1/camera2_2022-10-13_10-26-28.avi')  # 후방 차선 영상 파일
pcentc1, pcentc2 = [0, 0]
...
def frontCamFunc(frame, shiftArray):
    global ff, count1, track, start
    global pcent1, pcent2, pcent3, pcent4, pcent5
    global state1, state2, state3, state4, state5
    global y_block1, y_block2, y_block3, y_block4, y_block5
    ...
    # ff.write(str(count1) + ',' + str(track) + '\n')

    if len(shiftArray) < 10:
        shiftArray.append(track)

    laneTrackingResult = 0

    if len(shiftArray) == 10:
        shiftArray, laneTrackingResult = Lane_Tracking_v5_Run.LaneTracking(np.array(shiftArray))

    frame_num = str(count1)
    gap = str(cent5)
    track_num = str(track)
    ...
    cv2.putText(frame, str(start - int(wi / 2)), (wi - 80, hi - 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3, cv2.LINE_AA)

    count1 = count1 + 1
    frame = cv2.resize(frame, (261, 464))

    return frame, track, shiftArray, laneTrackingResult


def middleCamFunc(frame, shiftArray, laneTrackingBool):
    global count2
    global pcentc1, pcentc2
    global statec1, statec2
    global y_blockc1, y_blockc2, y_blockc3
    global statec1, statec2
    global trackc, startc
    global nozzle_onoff  # 초기 노즐 상태 변경(없음)
    global nozzle_state  # 초기 노즐 상태(닫힘)
    global disp, textcc, missed
    ...
    remain = 0
    line = cv2.line(frame, (int(wi / 2) - 50, int((hi * y_blockc1) / 100)), (int(wi / 2) + 50, int((hi * y_blockc1) / 100)), (255, 0, 0), 3, cv2.LINE_AA)
    line = cv2.line(frame, (int(wi / 2) - 50, int((hi * y_blockc2) / 100)), (int(wi / 2) + 50, int((hi * y_blockc2) / 100)), (255, 0, 0), 3, cv2.LINE_AA)
    line = cv2.line(frame, (int(wi / 2) - 50, int((hi * y_blockc3) / 100)), (int(wi / 2) + 50, int((hi * y_blockc3) / 100)), (255, 0, 0), 3, cv2.LINE_AA)
    start_end1, start_end2 = [False, False]
    centc1, centc2, abcent = [None, None, None]

    # 중앙 Guide_1(Blockc1) ==================================================
    blockc1 = gray[int((hi * y_blockc1) / 100):int((hi * y_blockc2) / 100), roi: wi - roi]
    ...
    for contc1 in contoursc1:
        ...
        if len(approxc1) == 4:
            ...
            if ((w > 110) and (w < 145)):
                pt1 = (x + roi + int(w / 2) - 2, y + int((hi * y_blockc1) / 100))
                pt2 = (x + roi + int(w / 2) + 2, y + int((hi * y_blockc1) / 100) + h)
                cv2.rectangle(frame, pt1, pt2, (0, 0, 255), -1)
                # if (h > (blockc1.shape[0] / 20)) and (h < int(blockc1.shape[0] - (blockc2.shape[0] / 20))): blockc2가 초기화되기 전에 들어올 경우 초기화에러
                #     start_end1 = True
                #     nozzle1 = (y, h)
                centc1 = int(pt1[0] + 2 - int(wi / 2))

    # 중앙 Guide_2(Blockc2) ==================================================
    blockc2 = gray[int((hi * y_blockc2) / 100):int((hi * y_blockc3) / 100), roi: wi - roi]
    ...
    for contc2 in contoursc2:
        ...
        if len(approxc2) == 4:
            ...
            if ((w > 130) and (w < 205)):
                pt1 = (x + roi + int(w / 2) - 2, y + int((hi * y_blockc2) / 100))
                pt2 = (x + roi + int(w / 2) + 2, y + int((hi * y_blockc2) / 100) + h)
                cv2.rectangle(frame, pt1, pt2, (0, 0, 255), -1)
                if (h > (blockc2.shape[0] / 30)) and (h < int(blockc2.shape[0] - (blockc2.shape[0] / 30))):
                    start_end2 = True
                    nozzle2 = (y, h)
                centc2 = int(pt1[0] + 2 - int(wi / 2))

    # 중앙 Absolute Position(Blockc3) ==================================================
    blockc3 = gray[(hi - 200):(hi), roi: wi - roi]
    ...
    for contc3 in contoursc3:
        ...
        if len(approxc3) == 4:
            ...
            if ((w > 170) and (w < 210)):
                pt1 = (x + roi + int(w / 2) - 10, y + (hi - 110))
                pt2 = (x + roi + int(w / 2) + 10, y + (hi - 90))
    ...
    if len(shiftArray) == 10:
        ...
        laneTrackingBool = True

    frame_num2 = str(count2)
    ...
    cv2.putText(frame, "Time(100ms): " + frame_num2, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(frame, gap2, (0, hi - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(frame, track_num2, (0, hi - 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(frame, abtext, (abcent + 20, hi - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2, cv2.LINE_AA)

    if nozzle_state == 0:
        textc = "Nozzle OFF"
    else:
        textc = "Nozzle ON"
    cv2.putText(frame, textc, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)

    if remain > 0:
        disp = 10
        textcc = "Nozzle state change: " + str(remain) + " (x10ms)"
    if disp > 0:
        cv2.putText(frame, textcc, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 3, cv2.LINE_AA)
        disp = disp - 1

    # 차선의 첫 시작 위치 지정(중앙) ==============
    if (startc == 0) and (centc2 != None):
        startc = centc2 + int(wi / 2)
    line = cv2.line(frame, (startc, 0), (startc, hi), (0, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(frame, str(startc - int(wi / 2)), (wi - 80, hi - 50), cv2.FONT_HERSHEY_SIMPLEX, 1,
                (0, 255, 255), 3, cv2.LINE_AA)

    count2 = count2 + 1
    frame = cv2.resize(frame, (261, 464))

    return frame, trackc, shiftArray, laneTrackingResult, laneTrackingBool, nozzle_state
